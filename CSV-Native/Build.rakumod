...
# it fails QUIETLY on purpose: the module runs its own Raku implementation
# instead, so a missing compiler or a non-Raku++ engine costs speed, never
...
class Build {
    method build($dist-path --> Bool) {
        ...
        unless $inc && $inc.add('rakupp/rakupp_ext.h').e {
            note "CSV::Native: no rakupp headers found; using the Raku implementation";
            return True;
        }
        ...
        unless $p.exitcode == 0 {
            note "CSV::Native: native build failed, using the Raku implementation";
