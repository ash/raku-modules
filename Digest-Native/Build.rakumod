...
# it fails QUIETLY on purpose: the module falls back to bduggan's two native
# dists plus Digest and Digest::HMAC, so a missing compiler or a non-Raku++
# engine costs speed, never function. A build hook that aborts the install
# would turn an optimisation into a dependency.

class Build {
    method build($dist-path --> Bool) {
        my $root = $dist-path.IO;
        my $src  = $root.add('src/digest.c');
        ...
        unless $inc && $inc.add('rakupp/rakupp_ext.h').e {
            note "Digest::Native: no rakupp headers found; using the ecosystem fallback";
            return True;
        }

        # -O2 is not decoration here. These are word-at-a-time loops over a
        # message schedule, and an unoptimised build of them runs several times
        # slower than the pure-Raku fallback is worth replacing.
        my @cmd = self!compiler, '-shared', '-fPIC', '-O2', "-I$inc",
                  |self!link-flags, $src.Str, '-o', $out.Str;
        my $p = run(|@cmd, :out, :err);
        unless $p.exitcode == 0 {
            note "Digest::Native: native build failed, using the ecosystem fallback";
