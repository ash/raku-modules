...
at `Variable '@список' is not declared`. Raku++ needs no such switch.
...
`rusku` with no arguments does the same thing, but it re-runs whichever
...
under both engines. Five differences remain, all of them the engine's, and
...
- **`-M` does not reach the REPL's own lines.** `rakupp -ML10N::RU -e '…'`
  ...
  L10N::RU;` at a prompt works on neither engine, but that one is the
