...
my ($out, $err, $rc) = child(
    'use Prompt::Hidden; my $x = prompt("pw: ", :hidden); say "[$x]"', "hunter2-a91c\n");
...
($out, $err, $rc) = child(
    'use Prompt::Hidden; say prompt("pin: ", :hidden).^name', "01234\n");
is $out, "pin: Str\n", 'an all-digit secret is a Str, not an IntStr';

($out, $err, $rc) = child(
    'use Prompt::Hidden; my $x = prompt("pin: ", :hidden); say $x eq "01234"', "01234\n");
is $out, "pin: True\n", 'and it keeps its leading zero';

($out, $err, $rc) = child(
    'use Prompt::Hidden; say prompt("pw: ", :hidden).^name', "");
is $out, "pw: Nil\n", 'end of input is Nil';

($out, $err, $rc) = child(
    'use Prompt::Hidden; my $x = prompt(:hidden); say "[$x]"', "bare-6d20\n");
is $out, "[bare-6d20]\n",
   'no message prints nothing — not the :hidden pair itself';

# Spaces and punctuation survive: `.get`, not a shell round trip.
($out, $err, $rc) = child(
    'use Prompt::Hidden; my $x = prompt(:hidden); say "[$x]"',
    "a b\tc\"d\$e'f-3e77\n");
is $out, "[a b\tc\"d\$e'f-3e77]\n", 'whitespace and shell metacharacters survive';

($out, $err, $rc) = child(
    'use Prompt::Hidden; my $x = prompt("pw: ", :hidden); say "[$x]"',
    "unicode-Ω-π-4c8b\n");
