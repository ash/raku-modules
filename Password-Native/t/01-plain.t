...
my ($out, $err, $rc) = child(
    'use Password::Native; my $x = prompt("who: "); say "[$x]"', "ada-4b1f\n");
is $rc, 0, 'plain prompt exits clean';
is $out, "who: [ada-4b1f]\n", 'message printed, line returned';

($out, $err, $rc) = child(
    'use Password::Native; say prompt("n: ").^name', "1234\n");
is $out, "n: IntStr\n", 'a numeric line still comes back an allomorph';

($out, $err, $rc) = child(
    'use Password::Native; say prompt("n: ").^name', "0x1f\n");
is $out, "n: IntStr\n", 'and a hex one too — val() is untouched';

($out, $err, $rc) = child(
    'use Password::Native; say prompt("n: ").^name', "not-a-number\n");
is $out, "n: Str\n", 'a non-numeric line is a plain Str';

($out, $err, $rc) = child('use Password::Native; say prompt("eof: ").^name', "");
is $out, "eof: Nil\n", 'end of input is Nil';

($out, $err, $rc) = child(
    'use Password::Native; my $x = prompt(); say "[$x]"', "no-message-9c2e\n");
...
($out, $err, $rc) = child(
    'use Password::Native; say prompt("n: ", :!hidden).^name', "1234\n");
